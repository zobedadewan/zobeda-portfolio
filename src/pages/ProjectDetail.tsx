...
export function ProjectDetail() {
  ...
  // Only the parts that have been written. A project listed before its
  // case study exists shows its header and nothing else, rather than three
  // empty headings.
  const sections = [
    ...
    <>
      <main className="pt-28 pb-24 lg:pt-36">
        <Container>
          <Link
            to="/#work"
            className="inline-flex items-center gap-2 font-mono text-[0.62rem] font-semibold tracking-widest text-ink-soft uppercase transition-colors hover:text-gold-deep"
          >
          ...
          <Reveal className="mt-8 max-w-3xl">
            ...
            {project.summary && (
              <p className="mt-5 text-base leading-[1.7] text-ink-soft">
                {project.summary}
              </p>
            )}
              ...
              <div className="mt-8 flex flex-wrap items-center gap-4">
                ...
                {project.repoUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer noopener"
                    className="inline-flex items-center gap-2 rounded-full border border-petal px-6 py-3 text-sm font-semibold text-ink transition-colors hover:border-rose hover:text-gold-deep"
                  >
                    Source
                    <GithubIcon size={16} />
                  </a>
            ...
            <div className="mt-16 grid gap-10 border-t border-petal pt-12 sm:grid-cols-3">
              {sections.map((section, index) => (
                <Reveal key={section.label} delay={0.06 * index}>
                  <h2 className="font-mono text-[0.62rem] font-bold tracking-widest text-gold-deep uppercase">
                    {section.label}
                  </h2>
                  <p className="copy mt-4 text-ink-soft">{section.body}</p>
                </Reveal>
              ))}
            </div>
          )}
